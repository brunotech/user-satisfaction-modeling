...
def train(args):
    ...
    logging.basicConfig(
        level=logging.DEBUG, filename=f'{save_path}/log.txt', filemode='a'
    )
    ...
    for i in range(args.epoch_num):
        logging.info(f'train epoch, {i}, {name}')
        ...
        if sat_test_result[-1] > sat_best_result[-1]:
            ...
            torch.save(model_to_save.state_dict(), f'{save_path}/best_pretrain.pt')
    ...
    checkpoint = torch.load(f'{save_path}/best_pretrain.pt')
...
def test(model, test_data, args):
    ...
    acc = sum(int(p == l) for p, l in zip(sat_prediction, sat_label)) / len(
        sat_label
    )
...
def evaluate(args):
    ...
    logging.basicConfig(
        level=logging.DEBUG, filename=f'{save_path}/log_eval.txt', filemode='a'
    )
    ...
    with open(f'{save_path}/results.txt', 'w') as outfile:
